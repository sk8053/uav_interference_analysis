...
parser.add_argument('--height',action='store',default=120,type= int,\
    help='uav_height')
...
for t in tqdm(range (run_time), desc= 'number of drops'):
   ...
   # print (n_uav_dropped)
        ...
        if bs.connected is True:
            ...
            if connection_complete is True:
                ...
                for c_ue in total_ue_list[connected_ue_ids]:
                    c_ue.compute_channels(codebook_bf= False)
    ...
    for bs in connected_bs_list:
        data = bs.get_interference(total_UE_list= total_ue_list,
                                   total_connected_ue_id_list = total_connected_ue_id_list, codebook_bf = True)
    ...
    if ptrl is True:
        if uav_access_bs_t is True:
            path_ul = ab_path + '/uav_interference_analysis/test_data/%d_stream_ptrl/cb_uplink_itf_UAV=%d_ISD_d_=%d_ns=%d_h=%d_%dG_%d.txt' % \
                (max_n, n_UAV, ISD_d, max_n, uav_height, int(frequency / 1e9), t)
        else:
            path_ul = ab_path + '/uav_interference_analysis/test_data_closed/%d_stream_ptrl/cb_uplink_itf_UAV=%d_ISD_d_=%d_ns=%d_h=%d_%dG_%d.txt' % \
                      (max_n, n_UAV, ISD_d, max_n, uav_height, int(frequency / 1e9), t)
    else:
        if uav_access_bs_t is True:
            path_ul = ab_path + '/uav_interference_analysis/test_data/%d_stream/cb_uplink_itf_UAV=%d_ISD_d_=%d_ns=%d_h=%d_%dG_%d.txt' % \
                    (max_n, n_UAV, ISD_d, max_n, uav_height, int(frequency / 1e9), t)
        else:
            path_ul = ab_path + '/uav_interference_analysis/test_data_closed/%d_stream/cb_uplink_itf_UAV=%d_ISD_d_=%d_ns=%d_h=%d_%dG_%d.txt' % \
                      (max_n, n_UAV, ISD_d, max_n, uav_height, int(frequency / 1e9), t)
