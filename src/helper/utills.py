...
def get_channels(ue_loc, bs_loc,channel_model, network, three_gpp=False, frequency = 28e9, cell_type = 1, aerial_fading = False):
    n_UE = len(ue_loc)
    if three_gpp is True:
        ...
        three_gpp_channel_list = channel_model.getList(data)

        three_gpp_channel_list = np.array(three_gpp_channel_list).reshape(n_UE, -1)
        return three_gpp_channel_list
    else:
        ...
        channels, link_states = channel_model.sample_path(dist_vectors, cell_types)
        channels, link_states = np.array(channels), np.array(link_states)
        channel_list, link_states_list = channels.reshape(n_UE, -1), link_states.reshape(n_UE, -1)

        return channel_list, link_states_list
