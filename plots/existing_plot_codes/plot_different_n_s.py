...
def get_df(dir_=None, isd_d = 200,  n_uav = 5, n_s = 1, uav = False,n_t = 10, feature ='SINR', height =60, ax = None):
    ...
    colors_= {1:'r', 2:'b', 3:'g', 4:'k'}
    lt_ = {1:'--',2:':',3:'-.',4:'-'}
    #colors_ = {0:'r', 5:'k'}
    #lt_ = {0:'-', 5:'-.'}
    if uav is True:
        plt.plot(np.sort(v_dict[feature]), np.linspace(0, 1, len(SINR)),lw = 2.5,  label = r'n$_s$ = %d'%n_s, color = colors_[n_s], linestyle = lt_[n_s])
        #plt.plot(np.sort(v_dict[feature]), np.linspace(0, 1, len(SINR)), lw=2.5, label=r'UAV = ' + str(n_uav*10) + '%',
        #         color=colors_[n_s], linestyle=lt_[n_s])
    else:
        plt.plot(np.sort(v_dict[feature]), np.linspace(0, 1, len(SINR)),lw= 2.5, label = r'n$_s$ = %d'%n_s,color = colors_[n_s], linestyle = lt_[n_s])
        #plt.plot(np.sort(v_dict[feature]), np.linspace(0, 1, len(SINR)),lw= 2.5, label=r'UAV = ' + str(n_uav*10) + '%',color = colors_[n_uav],
        #         linestyle =lt_[n_uav])
...
if uav is True:
    if closed is True:
        plt.savefig('uav_%s_120m_with_SDMA.png'%feature, dpi = 500)#%(feature,n_s, height), dpi = 500)
    else:
        plt.savefig('uav_%s_120m_with_SDMA.png'%feature, dpi = 500) #% (feature, n_s, height), dpi = 500)
else:
    if closed is True:
        plt.savefig('gue_%s_120m_with_SDMA.png'%feature, dpi = 500)#%(feature,n_s, height), dpi = 500)
    else:
        plt.savefig('gue_%s_120m_with_SDMA.png'%feature, dpi = 500) #% (feature, n_s, height), dpi = 500)
plt.show()
