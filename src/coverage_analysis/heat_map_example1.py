...
f = Heat_Map(mod_name='uav_lon_tok', bs_type = 'Terrestrial',
             npts=100, nsect = 3,
             horizontal_axis_of_net = np.linspace(-200, 200, 11), #41
             vertical_axis_of_net = np.linspace(0, 150,11),  plane_shift=0) #31
...
rx_matrix2= f.plot_heat_map(bs_type="Terrestrial", tilt_angle= -12, get_link_state=False, annot=False, nsect =3,
        plane_type= 'yz', bs_height=0)

np.savetxt('std_aod_theta.txt', rx_matrix2)
plt.show()