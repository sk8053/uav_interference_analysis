...
class power_optimization():
    ...
    def __init__(self, l_f=None, ue_ids=None, itf_no_ptx_list=None, itf_id_list=None, n_s = None):
        ...
        self.noise_power = 10 ** (0.1 * KT) * 10 ** (0.1 * NF) * self.BW
        # large scale gain
        ...
        for j in range(L):
            itf_no_tx_j = itf_no_ptx_list[j]
            f, b = itf_no_tx_j.find('['), itf_no_tx_j.find(']')
            itf_no_tx_j = itf_no_tx_j[f+1:b]

            itf_no_tx_j = np.array(itf_no_tx_j.split(','), dtype=float)
        ...
        for j in range(L):
            itf_ids=itf_id_list[j]
            f, b= itf_ids.find('['), itf_ids.find(']')
            itf_ids = itf_ids[f+1:b]
            itf_ids = np.array(itf_ids.split(','), dtype=int)
            ue_id = self.ue_ids[j]
    ...
    def target_function(self, P_tx):
        ...
        for j, ue_id in enumerate(self.ue_ids):
            ...
            SINR_lin =  self.l_f[j] * P_tx[j] / (I + self.noise_power)

            SINR_dB = 10*np.log10(SINR_lin +1) # plus 1 so that SINR>=0

            SINR_sum += np.log(SINR_dB)
    ...
    def get_optimal_power_and_SINR(self, minstep=1e-3, debug = False):
        ...
        #P = (ub+lb)/2
        #P_tx = minimize(self.target_function, P, tol = 1e-3 )
